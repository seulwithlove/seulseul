...
export async function getSatoriFonts(
	...
	// Pretendard는 Google Fonts에 없으므로 OG 이미지 생성 시 Noto Sans KR로 대체
	const headerFontName =
		typeof headerFont === "string" ? headerFont : headerFont.name;
	const bodyFontName = typeof bodyFont === "string" ? bodyFont : bodyFont.name;

	// Pretendard Variable을 Noto Sans KR로 대체
	const ogHeaderFont =
		headerFontName === "Pretendard Variable" ? "Noto Sans KR" : headerFontName;
	const ogBodyFont =
		bodyFontName === "Pretendard Variable" ? "Noto Sans KR" : bodyFontName;
	...
	// Fetch fonts for all weights and convert to satori format in one go
	const headerFontPromises = headerWeights.map(async (weight) => {
		const data = await fetchTtf(ogHeaderFont, weight);
		if (!data) return null;
		return {
			name: ogHeaderFont,
			data,
	...
	const bodyFontPromises = bodyWeights.map(async (weight) => {
		const data = await fetchTtf(ogBodyFont, weight);
		if (!data) return null;
		return {
			name: ogBodyFont,
			data,
