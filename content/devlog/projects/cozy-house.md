---
created: 2025-10-18, 14:46
updated: 2025-10-18T14:46
modified: 2026-03-12, 17:29
---
#devlog #project  #AI #application #python #fastapi #websocket #flutter 

# 포근한 집 Cozy House
> Computer Vision 활용 Flutter 어플리케이션

- 모두의연구소 아이펠 캠퍼스 코어과정 팀프로젝트
	-  [이슬](https://github.com/seulwithlove), [이선재](https://github.com/thetjswo), [김양희](https://github.com/claireyy01), [조수민](https://github.com/whtnals135)
- 2024. 2 ~ 2024. 4

[🔗 github repository](https://github.com/seulwithlove/AIFFELthon_CozyHouse/)


---
<div align="center">
    <img src="https://github.com/thetjswo/PJ_AIFFELthon/assets/140625136/3ad09459-7a9c-4c47-9f99-2834089aaf70" alt="Logo" width="200" height="200">
  </div>
## 프로젝트 소개 

> **포근한 집 Cozy House** 는 **1인 가구 대상 범죄를 예방하는 지능형 CCTV 앱**

- 외부인의 이상 행동을 탐지하고, 위험 등급별로 감지된 내역을 사용자에게 제공하여 보다 정확한 상황 파악이 가능
- 외부인 침입 전에 112에 신고하고 상황을 공유할 수 있어 신속한 대응으로 피해를 최소화
- 사용자가 능동적으로 상황을 주도할 수 있는 이 앱은 범죄에 대한 불안감을 해소하고, 집을 편안한 휴식 공간으로 만드는 것을 목표

### 담당 구현 기능
  - FastAPI, Websocket 기반 실시간 영상 스트리밍 및 실시간 객체 탐지 알림 기능
  - 데이터베이스에 저장된 영상의 재생 기능

<p align="center">
  <img src="https://github.com/user-attachments/assets/fccbf28c-f84a-4c7b-9dd8-66c60bca9d9c" alt="flow-of-ai-model" width="815" height="411">
</p>

<p align="center">
  <img src="https://github.com/user-attachments/assets/453f9dca-4208-41a6-8b46-949dc9871eb1" alt="flow-of-service" width="820" height="388">
</p>

### 앱 시연 영상
<p align="center"> <a href="https://youtu.be/8i5wmdSj_r0"> <img src="http://img.youtube.com/vi/8i5wmdSj_r0/0.jpg" alt="Video Label"> </a> </p>
